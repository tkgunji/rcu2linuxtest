// $Id: mrshellprim.c,v 1.19 2006/06/15 09:53:24 richter Exp $
...
int IsSeparator(char c, const char* pDelimiters)
{
  int iResult=0;
  if (c==0) {
    ...
    int i=0;
    for (i=0; pDelimiters[i]!=0; i++) {
      if ((iResult=(c==pDelimiters[i]))) {
	if ((g_mrShellPrimDbg&DBG_SCAN_ARG_DEF_DETAIL)==DBG_SCAN_ARG_DEF_DETAIL)
...
int SearchDef(const char* pArg, TArgDef* arrayDef, TFctMode *pMode)
{
  int iResult=0;
  if (pArg && arrayDef) {
    const char* pDelimiters=NULL;
    unsigned long flags=0;
    if (pMode) {
      pDelimiters=pMode->pDelimiters;
      flags=pMode->flags;
    }
    int i=0;
	...
	do {
	  ...
	  bSeparated=IsSeparator(pArg[iLen], pDelimiters);
...
int ScanArguments(const char** arrayArg, int iNofArgs, int iFirstArgStart, TArgDef* arrayDef, TFctMode *pMode)
{
  ...
  if (arrayArg && arrayDef) {
	...
	for (; iPos<=iSize && iResult==0; iPos++) {
	  ...
	  if (IsSeparator(pArg[iPos], pDelimiters)) {
